...
import type { AssessmentResults, AssessmentAnalysis, AptitudeData, ValueRatings } from '@/types/assessment'

const CAREER_FIELDS = {
  stem: {
    title: 'STEM (Science, Technology, Engineering, Math)',
    description: 'Careers in technology, engineering, mathematics, and scientific research',
  ...
  communication: {
    ...
    description: 'Careers in media, marketing, journalism, and public relations',
    careers: ['Marketing Manager', 'Journalist', 'Public Relations Specialist', 'Content Strategist'],
  },
  business: {
    title: 'Business & Finance',
    description: 'Careers in business management, finance, and entrepreneurship',
    careers: ['Business Analyst', 'Financial Advisor', 'Project Manager', 'Entrepreneur'],
  },
  healthcare: {
    title: 'Healthcare & Medicine',
    description: 'Careers in medical care, research, and wellness',
    careers: ['Registered Nurse', 'Physical Therapist', 'Medical Researcher', 'Healthcare Administrator'],
  },
  trades: {
    title: 'Skilled Trades',
    description: 'Careers in construction, repair, and hands-on technical work',
    careers: ['Electrician', 'HVAC Technician', 'Carpenter', 'Automotive Technician'],
  },
  socialServices: {
    title: 'Social Services & Education',
    description: 'Careers in teaching, counseling, and community support',
    careers: ['Teacher', 'Social Worker', 'School Counselor', 'Community Organizer'],
  },
  law: {
    title: 'Law, Policy & Public Service',
    description: 'Careers in legal services, government, and advocacy',
    careers: ['Lawyer', 'Policy Analyst', 'Police Officer', 'Environmental Advocate'],
  },
}
 ...
 * Identify top values
 */
function analyzeValues(values: ValueRatings): Array<{ value: string; score: number }> {
...
function analyzePersonality(personality: Record<string, number>): string[] {
  ...
  if (workEnv === 1) insights.push('You thrive in structured, traditional work environments')
  if (workEnv === 2) insights.push('You value flexibility and remote work opportunities')
  if (workEnv === 3) insights.push('You prefer outdoor or field-based work settings')
  if (workEnv === 4) insights.push('You enjoy variety in your work location')

  // Interaction style
  const interaction = personality.interaction_style
  if (interaction === 1) insights.push('You work best independently with minimal collaboration')
  if (interaction === 2) insights.push('You excel in small team environments')
  if (interaction === 3) insights.push('You thrive in larger team settings with clear roles')
  if (interaction === 4) insights.push('You energize from frequent interactions with new people')

  // Decision making
  const decision = personality.decision_making
  if (decision === 1) insights.push('You prefer data-driven, analytical decision making')
  if (decision === 2) insights.push('You trust your intuition when making decisions')
  if (decision === 3) insights.push('You value collaborative decision-making processes')
 ...
 */
function generateRecommendations(results: AssessmentResults, analysis: Partial<AssessmentAnalysis>): string[] {
  const recommendations: string[] = []
  const { basic, challenges } = results

  // Education-based recommendations
  if (basic.educationLevel === 'high-school' || basic.educationLevel === 'hs-graduate') {
    recommendations.push('Consider exploring community college programs or trade schools aligned with your interests')
  }
  if (basic.educationLevel === 'some-college' || basic.educationLevel === 'associates') {
    recommendations.push('Look into completing your degree or pursuing certifications in your top career fields')
  }

  // Financial considerations
  if (challenges.financial === 'very-limited' || challenges.financial === 'limited') {
    recommendations.push('Research scholarship opportunities and financial aid programs')
    recommendations.push('Consider careers with apprenticeship or on-the-job training options')
  }
  ...
  if (analysis.topCareerFields && analysis.topCareerFields.length > 0) {
    ...
    recommendations.push(`Explore entry-level positions in ${topField.field} to gain hands-on experience`)
  }
...
function generateNextSteps(results: AssessmentResults): string[] {
  ...
  if (results.basic.employmentStatus === 'unemployed' || results.basic.employmentStatus === 'student') {
    steps.push('Consider volunteer or internship opportunities to build experience')
  }
