...
function PersonalityPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    localStorage.setItem('assessment_personality', JSON.stringify(answers))
      ...
      <Container size="sm">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-stone-600">Section 2 of 5</span>
            <span className="text-sm font-medium text-stone-600">40% Complete</span>
          </div>
          <div className="w-full h-2 bg-stone-200 rounded-full overflow-hidden">
            <div className="h-full bg-lime-600 transition-all duration-500" style={{ width: '40%' }} />
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <Card className="mb-6">
            <CardHeader>
              <CardTitle className="text-2xl">Personality & Work Style</CardTitle>
              <p className="text-stone-600 mt-2">
                These questions help us understand your preferred work environment and style.
              </p>
            </CardHeader>

            <CardContent className="space-y-8">
              {questions.map((question, index) => (
                <div key={question.id} className="pb-6 border-b border-stone-200 last:border-0 last:pb-0">
                  ...
                  <div className="space-y-3">
                    {question.options.map((option) => (
                      ...
                      >
                        ...
                        />
                        <span className="text-stone-700 flex-1">{option.label}</span>
                      </label>
                    ))}
                  </div>
                </div>
